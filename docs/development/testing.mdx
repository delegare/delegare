...
Delegare provides a dedicated **Sandbox Environment** where you can safely test the entire spending delegate lifecycle—from session creation to atomic limit enforcement to dual-rail payments—without using real funds.
...
When your agent uses a test `delegateToken` to call `/charge`:
