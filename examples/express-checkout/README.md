...
Instead of redirecting users to a Stripe checkout page, your API allows an AI agent to execute a payment directly, programmatically, and safely using the user's spending delegate.
...
4. Delegare moves the funds from the user's card or crypto wallet to your Stripe account.
5. Your server completes the order!

## ⚙️ Setup

1. Copy `.env.example` to `.env`
2. Fill in your **Merchant ID** and **API Key** from your [Delegare Dashboard](https://app.delegare.dev)
...
## 🛍️ Generating a "Buyer" Delegate Token
...
2. Click the generated URL in your terminal.
3. Connect your Wallet (Crypto) or Credit Card (Fiat) in the browser UI.
4. Set your monthly spending limit for this "agent".
5. Return to your terminal! The script will automatically detect your completion and print out your new `intentMandate` (a signed JWT).

## 🧪 Testing the API

Now that you have your `eyJhbGci...` token, you can simulate your AI agent ordering a pizza by sending a POST request to your new server:
...
If the agent's token is valid and has enough budget, the payment will succeed, and the pizza will be "ordered"! Check your backend terminal logs to see the Delegare SDK processing the charge in real time.
